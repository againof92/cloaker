...
pub async fn create_pool(database_url: &str) -> Result<PgPool, sqlx::Error> {
    // Poolers (ex: Supabase PgBouncer) podem falhar com statement cache habilitado.
...
pub async fn migrate(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    for sql in stmts {
        if let Err(e) = sqlx::query(sql).persistent(false).execute(pool).await {
...
pub async fn load(pool: &PgPool, db: &AppDatabase) -> Result<(), sqlx::Error> {
    ...
    if let Some(row) = config_row {
        ...
        cfg.safe_page_url = row
            .get::<Option<String>, _>("safe_page_url")
            .unwrap_or_default();
    ...
    for row in link_rows {
        let link = RedirectLink {
            ...
            allowed_countries: decode_json_slice(
                row.get::<serde_json::Value, _>("allowed_countries"),
            ),
            blocked_countries: decode_json_slice(
                row.get::<serde_json::Value, _>("blocked_countries"),
            ),
...
pub async fn save_link(pool: &PgPool, l: &RedirectLink) -> Result<(), sqlx::Error> {
    ...
    .bind(&l.param_code)
    .bind(&l.offer_url)
    .bind(&l.safe_page_url)
    .bind(l.clicks)
    .bind(l.blocked)
    .bind(l.created_at)
    .bind(l.active)
    .bind(l.cloaker_active)
    .bind(l.ad_verify_mode)
    .bind(l.max_clicks)
    .bind(l.param_ttl)
    ...
    .bind(l.block_vpn)
    .bind(l.mobile_only)
    .bind(&l.allowed_hours)
    .bind(l.require_facebook)
    .bind(l.protection_total)
    .bind(l.strict_param_required)
    .bind(l.only_facebook_ads)
    .bind(l.advanced_fingerprint)
    .bind(l.ml_bot_detection)
    .bind(l.dynamic_referrer_spoof)
    .persistent(false)
    .execute(pool)
    .await?;
    Ok(())
...
pub async fn increment_link_stats(pool: &PgPool, link_id: &str, clicks: i32, blocked: i32) {
    if let Err(e) =
        sqlx::query("UPDATE links SET clicks = clicks + $1, blocked = blocked + $2 WHERE id = $3")
            .bind(clicks)
            .bind(blocked)
            .bind(link_id)
            .persistent(false)
    ...
    {
        error!(
            "[ERRO-DB] Falha ao incrementar stats link {}: {}",
            link_id, e
        );
    }
}
    ...
    sqlx::query(
        r#"INSERT INTO seen_ips (key, ip, link_id, first_seen, last_seen, attempts, blocked_at, user_agent)
           ...
           ON CONFLICT (key) DO UPDATE SET
             ...
             first_seen = COALESCE(seen_ips.first_seen, EXCLUDED.first_seen),
...
pub async fn get_logs(pool: &PgPool, limit: i64) -> Result<Vec<AccessLog>, sqlx::Error> {
    ...
    for row in rows {
        logs.push(AccessLog {
            ...
            region_name: row
                .get::<Option<String>, _>("region_name")
                .unwrap_or_default(),
            ...
            redirect_to: row
                .get::<Option<String>, _>("redirect_to")
                .unwrap_or_default(),
        });
    }
    Ok(logs)
